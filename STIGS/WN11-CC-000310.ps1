...
PS C:\> .\WN11_CC_000310.ps1 -WhatIf
#>
...
foreach ($view in $views) {
    ...
    if ($current -ne $desiredValue) {
        Set-RegistryDword -Path $regPath -Name $valueName -Value $desiredValue -View $view -WhatIf:$WhatIf
    } else {
        Write-Output "[$view] Already set to desired value ($desiredValue)."
    }
}

Write-Output "`nEnableUserControl setting applied: $desiredValue (0 = Disabled)"
